@extends('layouts.master', ['title' => 'Edit Product'])
@section('content')
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body p-4">
                ...
                <hr>
                <form class="form-body mt-4" method="POST" action="{{ route('admin.products.update', $product->id) }}"
                            ...
                            <div class="border border-3 p-4 rounded">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="slug" class="form-control" id="inputProductTitle"
                                        placeholder="Enter product Slug" value="{{ $product->slug }}">
                                    @error('slug')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" name="short_description" id="inputProductDescription" rows="3">{{ $product->short_description }}</textarea>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" class="form-control" id="inputProductDescription" rows="5">{{ $product->description }}</textarea>
                                    @error('description')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="row g-3">
                                    <div class="col-12">
                                        ...
                                        <input type="number" name="regular_price" step="any" class="form-control"
                                            id="inputPrice" placeholder="00.00" value="{{ $product->regular_price }}">
                                        @error('regular_price')
                                            <p class="text-danger">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="number" name="quantity" class="form-control"
                                            id="inputCompareatprice" placeholder="00" value="{{ $product->quantity }}">
                                        @error('quantity')
                                            <p class="text-danger">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select class="form-select" name="status" id="inputVendor">
                                            ...
                                            @foreach (collect(\App\enums\StatusProduct::cases())->toArray() as $status)
                                                <option @selected($status == $product->status) value="{{ $status }}">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select class="form-select" name="featured" id="inputCollection">
                                            <option selected disabled>--select Status --</option>
                                            @foreach (collect(\App\enums\StatusProduct::cases())->toArray() as $status)
                                                <option @selected($status == $product->featured) value="{{ $status }}">
